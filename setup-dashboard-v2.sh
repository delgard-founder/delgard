...
// Important : ne PAS lever d'erreur ici au chargement du module.
// Next.js importe les routes API pendant `next build` pour analyser
// leurs métadonnées (page data collection) — si DATABASE_URL manque
// à ce moment-là (ex: build sans variable d'environnement configurée),
// une erreur ici ferait échouer TOUT le build, même si personne n'a
// encore réellement besoin de la base de données.
// L'erreur claire arrive plutôt au moment d'une vraie requête, plus bas.
...
export const pool =
  ...
  new Pool({
    // Chaîne de secours : ne sert qu'à éviter un crash au chargement du
    // module. pg n'ouvre une vraie connexion qu'au moment d'une requête,
    // donc ceci ne fait rien tant qu'aucune requête n'est exécutée.
    connectionString: databaseUrl || "postgresql://placeholder:placeholder@localhost:5432/placeholder",
  });
...
/** À appeler en tête de chaque route API avant une vraie requête DB,
 * pour un message d'erreur clair plutôt qu'un crash de connexion cryptique. */
export function assertDatabaseConfigured() {
  if (!databaseUrl) {
    throw new Error(
      "DATABASE_URL n'est pas configurée. Ajoutez-la dans les variables d'environnement (fichier .env.local en développement, ou paramètres du projet en production) une fois votre projet Supabase créé."
    );
  }
}
DELGARD_FILE_EOF_9f3a2b
